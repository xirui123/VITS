...
def run(rank, n_gpus, hps):
  ...
  skip_optimizer = hps.model.freeze_textencoder
  try:
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      y_hat, l_length, l_pitch, l_energy, ids_slice, x_mask, z_mask, \
        (z, z_p, m_p, logs_p, m_q,
         logs_q), pred_f0, pred_norm_energy, norm_energy = net_g(phonemes, phonemes_lengths, f0, energy, phndur,
    ...
    if rank == 0:
      if global_step % hps.train.log_interval == 0:
        ...
        image_dict = {
          ...
          "all/energy": utils.plot_data_to_numpy(norm_energy[0, :].cpu().numpy(), pred_norm_energy[0,
                                                                                  :].detach().cpu().numpy()),
        }
        utils.summarize(
          writer=writer,
  ...
  with torch.no_grad():
    for batch_idx, (phonemes, phonemes_lengths,
      ...
      for shift, energy_shift in [(1, 1)]:
        ...
        # break
        y_hat, mask, xx, duration, pred_f0, energy = generator.module.infer(phonemes, phonemes_lengths,
                                                                            max_len=1000, sid=sid, pitch_control=shift, energy_control=energy_shift)
        ...
        image_dict.update({
          f"gen/mel-{batch_idx}": utils.plot_spectrogram_to_numpy(y_hat_mel[0].cpu().numpy()),
          f"all/f0-{batch_idx}": utils.plot_data_to_numpy(f0[0, :].cpu().numpy(),
